"""
Rebalances the VLUE-style value factor paper account.

Runs against its own Alpaca paper account + its own SQLite DB, configured
in .env.value (copy .env.value.example and fill in your own value-account
keys). Rebalances at most once every 80 days by default -- safe to invoke
daily from cron/scheduler, it'll just no-op most days.

Usage:
    python scripts/run_value_screen.py
    python scripts/run_value_screen.py --universe-limit 50   # quick test run
    python scripts/run_value_screen.py --force                # ignore the interval guard
"""
import argparse
...
load_dotenv(".env.value")

from src.factor_metrics import VALUE_METRICS
from src.factor_screener import run_strategy


def main():
    parser = argparse.ArgumentParser(description="Run/rebalance the VLUE-style value factor screen")
